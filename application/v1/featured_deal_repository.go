...
func (er *EventRepository) GetAllIncludingRelations(todayDateInGMT8 string) []*Event {
	...
	}).Preload("Deals.Items").Preload("Deals.Category").Preload("Deals.Items.Categories").Preload("Deals.Items.Subcategories").Preload("Deals.Grocerexclusives").
		Where(&Event{Status: "publish"}).Find(&events)
